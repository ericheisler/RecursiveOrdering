...
public class RecursiveOrder extends PApplet {
...
public void draw(){
  ...
  if(threeD){
    ...
    // Draw the nodes
    colorMode(HSB, round(nnodes*1.2f));
    strokeWeight(8);
...
public float maxDistance(int[] nod, int np){
  int minnod = nnodes;
  int maxnod = 0;
...
public void rescaleDepthIfNeeded(){
  dimx = xslider.getValue();
  dimy = yslider.getValue();
  if(threeD){
    dimz = zslider.getValue();
...
public int[] get_recursive_order(int type, int dim, int[] griddim){
    ...
    if(dim == 2){
        ...
        for(int i=0; i<fullnnodes; i++){ // i=1:length(rorder)
            ...
            int hind = rorder[i];
            int hx = hind%twotoN;
            int hy = round(floor(hind * 1.0f / (twotoN)));
        ...
        for(int i=0; i<fullnnodes; i++){ // i=1:length(rorder)
            ...
            int hind = rorder[i];
            int hx = hind%twotoN;
            int hy = round(floor((hind%(twotoN*twotoN)) * 1.0f / (twotoN)));
            int hz = round(floor(hind * 1.0f / (twotoN*twotoN)));
...
public int[] reorder_grid_recursive_3d(int[] grid, int[] griddim, int type){
...
public RecursiveOrdering morton_ordering_rules(int dim){
    if(dim == 2){
        int[][] tmp1 = {{1,1,1,1}};
        int[][] tmp2 = {{1,2,3,4}};
...
public RecursiveOrdering hilbert_ordering_rules(int dim){
    if(dim == 2){
...
public int[] rotation_3d(int[] a, int axis){
...
public int[] rotations(int[] a, int[] rots){
...
public int[] build_ordering_2d(RecursiveOrdering ord, int N, boolean invert){
    ...
    float centerx = (bbox[1]+bbox[2])*0.5f;
    float centery = (bbox[3]+bbox[4])*0.5f;
    float tmpcenterx = 0;
    float tmpcentery = 0;
    ...
    for(int ni=0; ni<nnodes; ni++){
        ...
        for(int i=0; i<N; i++){
            ...
            tmpcenterx = (tmpbbox[1]+tmpbbox[2])*0.5f;
            tmpcentery = (tmpbbox[3]+tmpbbox[4])*0.5f;
        }
...
// Build the ordering for a (2^N)^D grid
public int[] build_ordering_3d(RecursiveOrdering ord, int N, boolean invert){
    ...
    float centerx = (bbox[1]+bbox[2])*0.5f;
    float centery = (bbox[3]+bbox[4])*0.5f;
    float centerz = (bbox[5]+bbox[6])*0.5f;
    float tmpcenterx = 0;
    float tmpcentery = 0;
    float tmpcenterz = 0;
    ...
    for(int ni=0; ni<nnodes; ni++){
        ...
        for(int i=0; i<N; i++){
            ...
            tmpcenterx = (tmpbbox[1]+tmpbbox[2])*0.5f;
            tmpcentery = (tmpbbox[3]+tmpbbox[4])*0.5f;
            tmpcenterz = (tmpbbox[5]+tmpbbox[6])*0.5f;
        }
...
public int[] invert_ordering(int[] ord, int nnodes){
...
public int[] tiled_order_2d(int[] griddim, int[] tiledim, boolean invert){
...
public int[] tiled_order_3d(int[] griddim, int[] tiledim, boolean invert){
    // griddim and tiledim are tuples of grid dimensions
    int gridx = griddim[0];
    int gridy = griddim[1];
    int gridz = griddim[2];
...
class Button{
  ...
  public void centerText(boolean doit){
    centerTextX = doit;
  }
}
...
class Label{
  ...
  public void showBox(boolean doit){
    withBox = doit;
  }
}
...
class Slider{
  ...
  public void display(){
    ...
    strokeWeight(1);
    fill(bColor);
    int pos = bartop + barlength - round(barlength * (val-rangemin)*1.0f/(rangemax-rangemin));
  ...
  public void setRange(int mini, int maxi){
    rangemin = mini;
    rangemax = maxi;
  }
  ...
  public void clickSlide(){
    ...
    py = min(py, barlength);
    py = max(py, 0);
    int newval = rangemin + round((rangemax-rangemin)*py*1.0f/barlength);
  ...
  public void settings() {  size(1200, 850, P3D); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "RecursiveOrder" };
    if (passedArgs != null) {
